...
real(dp) :: dx,dy,dz,rij2,rij8,rij14,forcedist,potdist,rij12,rij6,epot
real(dp),intent(in), dimension(N,dim) :: x,v
real(dp), dimension(N,3) :: F
real(dp),intent(out), dimension(N,dim) :: Ftot
real(dp),intent(out) :: resepot
...
call MPI_ALLREDUCE(F,Ftot,size(Ftot),MPI_REAL8,MPI_SUM,MPI_COMM_WORLD,ierror)

! potential energy global
call MPI_REDUCE(epot, resepot, 1, MPI_REAL8, MPI_SUM, 0, MPI_COMM_WORLD,ierror)
