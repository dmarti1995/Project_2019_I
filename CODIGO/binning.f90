...
!SUBRUTINA QUE REALITZA EL BINNING DE L'ENERGIA.
...
!ES CALCULA EL VALOR ESPERAT I LA VARIANÇA DE E I M AMB AQUEST NOU RESAMPLEIG
	...
	write(432,'(i6,100f20.12)') m,ValEc,sqrt(VarEc),ValEp,sqrt(VarEp),ValEm,&
		sqrt(VarEm),ValP,sqrt(VarP),ValT,sqrt(VarT)
	deallocate(AUXP)
	deallocate(AUXT)
	deallocate(AUXEc)
...
integer,parameter ::  Nini = 0
integer           :: timesteps, Npart, dummy

! Read number of time steps of the simulation and the number of particles
...
call BINNING_SUBR(2,"data_EK_EP_T_P.dat","binning.dat",timesteps,Nini,Npart)
